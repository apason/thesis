...
% are sometimes referred interchangeably. 
...
consecutive duplicate strings. The keywords are sorted using a string quickstort \ref{asd}.
...
Our implementation can be found from GitHub: https://github.com/apason/ukkonen-90
...
a compile time macro that changes the data structure that is used to save the the children of each
...
\paragraph{Compilation and usage:}~\label{usage}


\paragraph{Tests:}


%% Note that the Makefiles are specifically written for GNU Make and does not necessarily work with other Make tools.
...
%% LONG_KEYS. oletuksena depth funktio palauttaa uint8 muuten uint16
%% max line määrittää yhden rivin maksimipituuden.

%% -limitations: fixed size alphabets (no utf-8)

\section{Benchmark Data}
  %HW + instances

example of the time fractions in our implementation!

- gnu time
-pizzachili/real. Rivinvaihdot korvattu tabulaattoreilla.
- DNA. Poistettu N kohdat kokonaan
- dnassa myös paljon duplikaatteja jotka on poistettu
