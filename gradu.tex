% TODO: find and replace. node vertex state ?!?!

%% TODO: todo todo todo

%% footnotet oikeilla sivuilla
...
\begin{algorithm}[t]
  ...
  \hspace*{\algorithmicindent} \textbf{Preconditions:} Initially $g(s,\sigma)$ is undefined for each state and symbol
      ...
      \item[]
        ...
        \Function{insert}{$s = \sigma_0,\sigma_1,...,\sigma_{m-1}$}
          \State $state\gets 1$
          \State $j\gets 0$
          \While{$g(state, \sigma_j)$ is defined}
...
%   [scale=.8]
  ...
  \end{tikzpicture}
  \caption{The failure function construction when $f(q)$ is defined for all states $q$ with depth $\leq$ 1.} \label{fig-ac-step5}
\end{figure}
      ...
      \Function{calculateFailureFunction}{$g$}
        ...
        \While{$queue \neq empty$}\\
          ...
          \For{each $\sigma$ s.t. $g(r,\sigma) = q$ is defined}
            ...
            \While{$g(state,\sigma)$ is undefined}
