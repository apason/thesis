...
\begin{abstract}
  ...
  % OUR contribution..
  For a set of strings a \textsc{Shortest Common Superstring} is a string
  for which each member of the set is a substring of.
  Since the optimization problem of finding a \textsc{Shortest Common Superstring}
  is \textsc{NP}-hard, a few heuristics are developed to solve the approximation version of
  this problem.
  \textsc{Shortest Common Superstrings} have many applications in the fields of
  data compression and bioinformatics.
  
  We provide an implementation of the Ukkonen's linear time alogorithm for the
  greedy heuristic. The practical performance of this implementation is measured by
  comparing it to another implementation of the same heuristic. The potentiality
  of the usage of the \textsc{Shortest Common Superstring} approximation with
  Relative Lempel-Ziv compression algorithm is also verified.
...
the computation. In fact, depending on the chosen datastructures, Ukkonen's algorithm can be implement
