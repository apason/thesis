...
%
% 0.50_512 time, 0.75_512 time,
% Number of factors ok koska voidaan käyttää omee implee.
%
...
\chapter{Terminology and Problem Definitions}~\label{chp-preli}
...
%  \hspace*{\algorithmicindent} \textbf{Method:} The index of the root node is 1. The procedure \textit{insert($s$)} inserts into the goto graph a path that spells out $s$ or does nothing if there already exists such a path.
  ...
  Algorithm 1 creates a goto function. It begins by setting the start state on line 2.
  ...
  in the line 9 keeps track of this prefix. After the first symbol not already represented in the
...
%   [scale=.8]
  ...
  \end{tikzpicture}
  \caption{The complete goto graph.} \label{fig-ac-goto}
\end{figure}
  \end{example}
...
%   [scale=.8]
  ...
  \end{tikzpicture}
  \caption{The failure function construction when $f(s)$ is defined for all states $s$ with depth <= 1.} \label{fig-ac-step5}
\end{figure}
...
\end{figure}
  ...
  \ref{lst-failure-calculation} describes. On line 7 and 8 we pop a
  state from the queue. At this point the failure function for that state is already defined. Then we
  process each child of the state separately on lines 9 to 14 by \circled{1} adding the child to the
  queue  on line 10  \circled{2} travelling the failure function of the parent as long as we get a state
  that have goto   transition with the same symbol that appears between the parent and the currently
  processed child on lines 12 to 13 and  \circled{3} set the failure function for the child on line 14.
...
line 9 are executed once for every state, that is $O(n)$ times. The lines 10 and 11 are clearly constant
time operations and the time complexity of line 14 depends on the goto function implementation and
...
TODO ... Got stuck here..? How to prove that construction of $f$ is in linear time?
...
as discussed before. Finally the line 23 sets $F(i) = 1$ for each keyword index $i$ such that
the state represented by $s_i$ has an incoming failure transition from any other state.

The lines 12 together with line 23 ensure that for all keywords $i$ such that $s_i$ is a substring
...
a linked list). The lines 21 to 23 that are not related to the BFS search itself can also clearly
...
After that each keyword index in $L(s)$ that is not forbidden are processed (line 12). Each
...
On line 21 the edge is selected. The line 22 ensures the definition for $forbidden$ function
remains valid and lines 24 and 25 updates the $FIRST$ and $LAST$ functions accordinly. On line
...
todo: How is that condition on line 12 resolved in linear time ????
