...
In this chapter we describe Ukkonen's linear time approximation algorithm \ref{Ukkonen90} for
...
The compositions of $p(H)$ and a shortest common superstring $z$ are equivalent.
\end{lemma}
...
\begin{theorem} [] ~\label{lem-similarity-between-SCS-Hamp}
  ...
  a shortest common superstring for $R$.
...
\end{figure}
  ...
  in Listing \ref{failure-calculation}. Since the AC-machine does not save parent information
  this is done a bit differently, although the result is exactly the same as Listing
  \ref{failure-calculation} describes. On line 8 and 9 we pop a
  node from the queue. At this point the failure function for that node is already defined. Then we
  process each child of the node separately on lines 10 to 17 by 1) adding the child to the queue
  on line 11  2) travelling the failure function of the parent as long as we get a node that have goto
  transition with the same symbol that appears between the parent and the currently processed child
  on lines 12 to 15 and  3) set the failure function for the child on line 16. The execution of the
...
children of the root node. Since the root node has only $|\Sigma|$ goto links the loop is executed at
most $|\Sigma|$ times. In the worst case every symbol
...
Let us examine Algorithm \ref{ac-fail}. In the discussion of Theorem
...
the maximal overlap between $p$ and $r_j$ has length $d(q)$. For example, the set $L(3) = 1,2$
