...
\chapter{Ukkonen's Algorithm}
 \listofalgorithms
  ...
  \begin{algorithm} % for captioning
    ...
    \hspace*{\algorithmicindent} \textbf{Method:} We assume \textit{output($s$)} is empty when state $s$ is first created, and $g(s,a) = fail$ if $a$ is undefined or if $g(s,a)$ has not yet been defined. The procedure \textit{enter($y$)} inserts into the goto graph a path that spells out $y$.
