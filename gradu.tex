...
%Text algorithms occur in many areas of science and
...
of molecular sequences. The complexity of text algorithms is also one of the
central and most studied problems in theoretical computer science.

The basic textual problem in stringology is called pattern matching.
-string matching
-data compression
...
from its compressed from. No loss of information is allowed.
...
documents via telephone network, for example.
General compression methods often adapt themselves to the data.
The compression is very successful if there are redundancies and regularities
in the information message. The analysis of data is related to the problem of
detecting regularities in texts.

% molecular biology
DNA = STRING
amino = string
In a first approach all these molecules can be viewed as texts.
The
discovery twenty years ago of powerful sequencing techniques has led to a rapid
accumulation of sequence data. Prom the collection of sequences up to their
analysis many algorithms on texts are implied. Moreover, only fast algorithms
are often feasible because of the huge amount of data involved.

typical two di-
mensional pattern-matching problem in two dimensions. The reconstruc-
tion of a whole sequence from small segments, used for instance in the shotgun
sequencing method, is another example of a problem that occurs during this
step. This problem is called the shortest common superstring problem:
construction of the shortest text containing several given smaller texts.

The result is know
as an alignment of the set of nucleotides. Alignment of two sequences is the
edit distance problem: compute the minimal number of edit operations to
...
% Aho corasick
- in many information retrieval and text-editing applications it is necessary to
be able to locate quickly some or all occurrences of user specified patterns or
words and phrases in text.

% ukkonen90
DNA sequencing leads to a fragment assembyl problem whose natural abstract formulation comes
close to SCS problem \citep{Peltola83}, \citep{Peltola83}
...
% Intron intro
Textually represented data -- text -- is one of the most common type of preserved information.
In stringology, a field of computer science that studies text processing and manipulation,
this kind of data are modeled as a finite sequences of symbols called strings. Stringology covers
...
than the input keywords, it can be seen as a packed form of the input. 

thus decreasing the redundancy

%molecular biology


As its decision version is \textsc{NP}-complete \citep{Gallant80}, several heuristics have been
...
is shorter compared to the lengths of the input keywords combined. For the greedy heuristic, first
...
which is used to implement the heuristic. In addition we prove the validity of Ukkonen's algorithm
and discuss the time complexities of all of its sections. The concept of dictionary compression is
...
- \citep{Turner89} showd this is false.
