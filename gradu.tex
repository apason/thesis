...
\begin{theorem}[]~\label{thm-time-h}
  The time complexity of Algorithm \ref{ukk-h} is same as the time complexity of
  Algorithm \ref{ac-goto}
\end{theorem}
\begin{proof}[Proof scketch]
...
starting at line 10 iterates once for each state that is $O(n)$ times. However, the for loop starting
at line 12 is also executed at most $m = O(n)$ times.

\end{proof}

Theorems \ref{thm-time-goto}, \ref{thm-time-failure}, \ref{thm-time-pre} and \ref{thm-time-h}
show that every algorithm presented in this chapter has the same time complexity depending
