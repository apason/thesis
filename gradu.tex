...
\begin{lemma}[]~\label{lem-maximal-overlap}

  The maximal overlap between each $s_i$ and $s_{i+1}$ is $v_i$

  \textit{Proof:} Lets assume the opposite. Let $w$ be a suffix of $s_i$ which is longer than $v_i$.
  Now $s_i$ can be written as $w$
...
\begin{theorem} [] ~\label{lem-similarity-between-SCS-Hamp}
  ...
  The projection $p(H)$ equals with the shortest common superstring of $R$, when $H$
  is the longest Hamiltonian path in the overlap graph for $R$.

\end{theorem}

The following defines a conceptually equivalent greedy heuristic for the construction of the shortest common
superstring approximation by construction of the approximation of the longest Hamiltonian path in
an overlap graph.
...
\begin{enumerate}[i]
\item The selection does form a cycle.
...
%   [scale=.8]
  ...
  \end{tikzpicture}
  \caption{An example of an AC-machine. TODO: 4->11 arrow head is missing.} \label{fig-ac}
\end{figure}
