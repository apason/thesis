...
A concepts of a prefix and a suffix is used to define an overlap. 
...
\begin{definition}[\textsc{Overlap and Maximal Overlap}]~\label{def-overlap}\\
  ...
  If $U$ is a longest such string, it is said to be maximal overlap.
\end{definition}
...
Note that by Definition \ref{def-sub-super}, prefixes and suffices are also substrings.
...
If a string contains (I.e. is a superstring) each string in a set of strings, it is said to be
...
\begin{example}~\label{exml-cs}\\
  ...
  A string ``baababaababaab'' is a common superstring of the set $R$.
\end{example}
...
If a common superstring is a shortest possible it is said to be a shortest common superstring (SCS).

\begin{example}~\label{exml-scs}\\
  Let $R=\{$ ``baa'', ``baba'', ``abab'', ``aab''$\}$ be a set of strings.
  A string ``aababaa'' is an shortest common superstring of the set $R$.
\end{example}
...
is same as the SCS of the same set of strings that is not reduced. 
...
problem in a weighted digraph. For this reason, we will also define a necessary terminology
to understand the latter problem. The proof of the reducibility is given in Chapter \ref{chp-ukkonen}.

Regardless of the type of a graph, a Hamiltonian path in a graph is a path (sequence of connected nodes)
that contains every node in the graph exactly once. Formally,
...
For a weighted graph a longest Hamiltonian path is a Hamiltonian path of a maximum length.
...
\begin{definition}[\textsc{Longest Hamiltonian Path Problem}]~\label{def-longest-hampat-problem}\\
  Given a weighted graph $G$, a longest Hamiltonian path problem asks to find a
...
greedy heuristic was to examine the practical performances
