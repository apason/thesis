% TODO: 2x explanation of algorithm 3 ???

%% TODO: todo todo todo

%% TODO. testexamplen reffit väärillä numeroilla?! figuren ja lemman numerot samat??

%% set of keywords K or R ? be consistent!
%% strings and states with lower case letters ? 

% textsc for problems.

% label sekä tyhjä rivi välissä example, def, lemma .... vaikuttaa rivivaihtoon? konsistenssi?
...
% ACM (sart) state, goto and failure transitions
...
\usetikzlibrary{positioning,arrows.meta,shapes,automata}
...
\newtcolorbox[use counter=theorem, number within=chapter]{testexamplebox}{%
    % Example Frame Start
...
\examiners{Prof.~Leena Salmela}


\keywords{Implementation, Shortest Common Superstring, Relative Lempel-Ziv}
...
%molecular biology % lähteet!!??!? fragment assemblyyn lähteitä gusfieldin apsp paperista.
...
% introduction to heuristics of scs. viitataan aiempiin ongelmiin ja todetaan että ei tarvi parasta
...
at most $2n$ overlaps need to be examined. This significantly decreases the time complexity of
...
The length of the string equals the number of symbols it contains. Sequences of symbols
...
\begin{definition}[\textsc{String}]~\label{def-string}\\
  ...
  The length of the string $S$ denoted by $|S|$ is the number of symbols in the string.
  An empty string denoted by $\varepsilon$ contains zero symbols. An $i$th symbol of a string
  is denoted by $S[i]$.
\end{definition}
...
A concepts of a prefix and a suffix are used to define an overlap. 
A string $S_p$ is a prefix of string $S$ if the first $|S_p|$ symbols of $S$ spells out $S_p$.
Similarly, a string $S_s$ is a suffix of string $S$ if the last $|S_s|$ symbols of $S$ spells out $S_s$.
Note that the prefix and the suffix may also be empty.

\begin{definition}[\textsc{Prefix}]~\label{def-prefix}\\
  Let $S$ and $S_p$ be strings. The string $S_p$ is a prefix of $S$ if and only if
  $S = S_p\cdot S'$ for some string $S'$.
\end{definition}

\newpage

\begin{definition}[\textsc{Suffix}]~\label{def-suffix}\\
  Let $S$ and $S_s$ be strings. The string $S_s$ is a suffix of $S$ if and only if
  $S = S'\cdot S_s$ for some string $S'$. 
\end{definition}
...
\begin{definition}[\textsc{Overlap and Maximal Overlap}]~\label{def-overlap}\\
  Let $S$ and $T$ be strings. A string $U$ is an overlap between $S$ and $T$
  if and only if $U$ is a prefix of $S$ and a suffix of $T$.
  If $U$ is the longest such string, it is said to be maximal overlap.
\end{definition}

If a string $S$ contains another string $S'$ we say that the string $S'$ is a substring of a
...
\begin{definition}[\textsc{Substring and Superstring}]~\label{def-sub-super}\\
  ...
  The string $S'$ is a substring of $S$ if and only if $S$ can be written as $R\cdot S'\cdot T$,
  where $R$ and $T$ are strings. $R$ and $T$ may be empty.
...
\begin{definition}[\textsc{Common superstring}]~\label{def-cs}\\
  Let $K = \{s_1,...,s_n\}$ be a set of strings and $T$ be a string.
  $T$ is a common superstring of $K$ if and only if $T$ is a superstring
  for each $s_i$ such that $i\in 1,...,n$.
\end{definition}
...
\begin{testexample}~\label{exml-cs}\\
  Let $K=\{$ ``baa'', ``baba'', ``abab'', ``aab''$\}$ be a set of strings.
  The string ``baababaababaab'' is a common superstring of the set $K$.
\end{testexample}
...
\begin{testexample}~\label{exml-scs}\\
  Let $K=\{$ ``baa'', ``baba'', ``abab'', ``aab''$\}$ be a set of strings.
  A string ``aababaa'' is a shortest common superstring of the set $K$.
\end{testexample}
...
\begin{definition}[\textsc{Shortest Common Superstring}]~\label{def-scs}\\
  ...
  The string $T$ is a shortest common superstring of $K$ if and only if there does not exist
  a common superstring of $K$ which is shorter than $T$.
\end{definition}
...
\begin{definition}[\textsc{Reduced Set of Strings}]~\label{def-reduced}\\
  Let $R=\{S_1, S_2,...,S_n\}$ be a set of strings. The set $R$ is called reduced
  if and only if $S_i$ is not a substring of $S_j$ for all $i,j = 1,...,n, i\neq j$.
\end{definition}
...
\begin{proof}
  ...
  Therefore $S_{scs}$ is also a shortest common superstring of a set
  $R$.
\end{proof}
...
\begin{definition}[\textsc{Shortest Common Superstring Problem}]~\label{def-scs-problem}\\
  Given a set of strings $R$, a shortest common superstring problem
...
\textsc{Shortest Common Superstring} problem by reducing it to a \textsc{Longest Hamiltonian Path}
...
For any given graph, a Hamiltonian path in a graph is a path (sequence of connected nodes)
that contains every node in the graph exactly once.

\begin{definition}[Hamiltonian Path]~\label{def-hampath}\\
  Let $G=(V,E)$ be a graph. A hamiltonian path in $G$ is a $|V|$ sized sequence of nodes
  that contains each node in $V$ exactly once and for each consecutive nodes $v_i$ and
...
\begin{definition}[Longest Hamiltonian Path]~\label{def-longest-hampath}\\
...
Finally, we define a \textsc{Longest Hamiltonian Path} problem as follows
...
as an instance of a longest Hamiltonian path problem. The encoding works by creating a weighted
...
\item The earlier selected edges can not have the same start node as $e$.
\item The earlier selected edges can not have the same end node as $e$.
\end{enumerate}
...
$s_d$ with the current input symbol $c\in \Sigma$ of the state automaton. If no such
...
%  \hspace*{\algorithmicindent} \textbf{Method:} The index of the root node is 1. The procedure \textit{insert($s$)} inserts into the goto graph a path that spells out $s$ or does nothing if there already exists such a path.
      ...
      \begin{algorithmic}[1]
        ...
        \Function{calculateGotoFunction}{$K$}
  ...
  10 to 12 is to skip the prefix of $s$ that already exists in the trie. The variable $j$ initialized
  in line 9 keeps track of this prefix. After the first symbol not already represented in the
...
%%   \end{tikzpicture}
%%   \caption{A root node.} \label{fig-ac-step1}
%%   \end{figure}
...
%   [scale=.8]
    ...
    \draw[-{Stealth[length=1.5mm,width=3mm]}]
    (1) edge node[above] {b} (2)
    (2) edge node[above] {a} (3)
    (3) edge node[above] {a} (4);

  \end{tikzpicture}
  \caption{First key added.} \label{fig-ac-step2}
  \end{figure}
...
%   [scale=.8]
  ...
  \end{tikzpicture}
  \caption{Second key added.} \label{fig-ac-step3}
\end{figure}
  ...
  %Figure \ref{fig-ac-step1} presents this state.
  ...
  function executes the while loop two times since the common prefix of ``baa'' and ``baba'' has length
...
%   [scale=.8]
  ...
  \end{tikzpicture}
  \caption{The complete goto graph for the set of keywords $K=\{$``baa'', ``baba'', ``abab'', ``aab''$\}$.} \label{fig-ac-goto}
\end{figure}

  Let us define a depth function for the AC-machine. At this point depth is merely a concept of the
  machine, but later on it becomes a part of the data structure in Algorithm \ref{ukk-pre}.
  In an AC-machine the depth of a state $p$, denoted by $d(p)$, is the depth of the node (number of
  edges from root to that node) that corresponds
  the state in the goto graph. This is equivalent to the length of the string that represents the
  ...
  \begin{testexample}[]~\label{exmp-fail}\\
    ...
    is not defined. For the states of depth 1 ($s\in\{2,7\}$) the failure function is defined as
    $f(s)=1$. Figure \ref{fig-ac-step5} presents this state.
...
%   [scale=.8]
    ...
    %\path [->,dotted]
    \draw[-{Stealth[length=1.5mm,width=3mm]},dotted]    

    (2) edge[bend left=15] (1)
    (7) edge [bend right=15] (1);
    
  \end{tikzpicture}
  \caption{The failure function construction when $f(s)$ is defined for all states $s$ with depth $\leq$ 1.} \label{fig-ac-step5}
\end{figure}
  

  In Example \ref{exmp-fail} the value for the failure function was always found in a single
  iteration of the step 3 of the above definition (Figure \ref{lst-failure-calculation}). Next we go through the pseudocode for the failure
  ...
  \begin{algorithm}[]
    ...
    \begin{algorithmic}[1]
      \Function{calculateFailureFunction}{$g$}
...
%   [scale=.8]
    ...
    \draw[-{Stealth[length=1.5mm,width=3mm]},dotted,rounded corners, shorten <= 0.5cm, shorten >= 0.5cm] plot coordinates {(4) (v1) (v2) (11)};
    % repaint these nodes
    \node[gnode, above right = 1cm and 2.277cm of 3] (4) {$4$};
    \node[gnode, below right = 1cm and 2.277cm of 7] (11) {$11$};
  ...
  \end{tikzpicture}
  \caption{Fully constructed AC-machine for the keywords $R=\{$"baa", "baba", "abab", "aab"$\}$.} \label{fig-ac}
\end{figure}
...
\begin{proof}[Proof (by induction)]
  ...
  According to the induction hypothesis, $u_1$ is the longest (or equal longest) proper suffix of $x_1x_2\cdots x_{d-1}$
...
function maps each state (except the root) to some other state. It is effective and practical
to implement this as an array which stores the value $f(s)=d$ such that the array index $s$ has
a value $d$. Therefore both inserting to and reading from the failure function is performed
...
Let us next examine the insert function. The first two lines 8 and 9 can clearly be executed in
constant time. The first while loop is used to find the symbol of the string that is not yet defined
in the goto function. Let $s=vu$ be a string such that $v$ is the part of $s$ that is already represented in the
...
only for $u$. In the other words the \textit{insert} function behaves such that every symbol of
...
$O(1)$ instructions to the for loop in the \textit{insert} function. The body of this for loop
  ...
  \begin{algorithm}[!ht]
    ...
    \begin{algorithmic}[1]
      \Function{calculateAuxiliaryFunctions}{$g, f, R$}
        \For {$i = 1,...,m$} \\
          \hspace*{1.1cm plus \algorithmicindent} let $s_i = a_0,...,a_{k-1}$
          \State $s\gets 1$
          \For {$j = 0,...,k-1$}
            \State $s\gets g(s,a_j)$
            %% ERRATA: i = j.
            \State $L(s)\gets L(s) \cdot \{i\}$ \footnotemark[1]
            \If {$j = k-1$}
              \State $F(i)\gets s$
              \State $E(s)\gets i$
...
of some other keyword. (lines 11, 12).
  ...
  \footnotetext[1]{In the original algorithm this line was $L(s)\gets L(s) \cdot \{j\}$}
  \footnotetext[2]{To prevent infinite recursion the inequality $s\neq r$ was added}
...
the edge in question must be discarded. 
  ...
  \begin{algorithm}[!ht]
    ...
    \hspace*{\algorithmicindent} \textbf{Input:} The AC-machine and the output from Algorithm \ref{ukk-pre}, set of keywords $R=\{s_1,...,s_m\}$ \\
    \hspace*{\algorithmicindent} \textbf{Output:} The approximate longest Hamiltonian path $H$ in the overlap graph.
    \begin{algorithmic}[1]
      \Function{createPath}{$f,F,L,b,B,R$}
        \For {$j = 1,...,m$}
          \If {$F(j)\neq 1$}
        ...
        \While {$s\neq 1$}
          \If {$P(s)$ is not empty}
            \For {each $j$ in $L(s)$ s.t. $forbidden(j) = false$}
              ...
              \If {$P(s)$ is empty} \footnotemark[1]
                \State \textbf{break}
              \EndIf
...
first state to be processed is set on line 9. Note that setting $s=B$ would cause one more unnecessary
...
On line 11 we ensure that $P(s)$ is not empty since otherwise there are no allowed overlaps.
\footnotetext[1]{This line is not present in the original algorithm}
After that each keyword index in $L(s)$ that is not forbidden is processed (line 12). Each
iteration (except the last one if no allowed edge is found) of that for loop decreases the
size of $P(s)$ and $\{i\in L(s) : forbidden(i) = false\}$. On lines 13 to 14 we check that
the set $P(s)$ to ensure there is at least one possible startpoint to the new edge. After line 15
...
another index in $P(s)$ and, if so, the edge is created. Otherwise the execution jumps to the
next iteration. Note that if the edge $(s_i,s_j)$ would create a cycle then no other edge $(s',s_j)$
will for any $s'$. Therefore it $P(s)$ only needs to be queried at most twice with each iteration.
...
23 the selected keyword index is removed from $P(s)$. After all possible edges starting from
$s_{P(s)}$ are examined, the process can move on to the next state in the $b$ link chain (line 27).
Before that, on line 26, the $P(s)$ is concatenated to $P(f(s))$ so the possibly remaining unused
...
total length of supporters sets $|L(s)|$ over all states $s$. Although $L(s_i)$ may contain
(partly or completely)
the same states as $L(s_j), i\neq j$, the sum $\sum_s |L(s)|$ is $O(n)$. Specifically,
$\sum_s |L(s)| = n$ for a reduced set of input keywords and $\sum_s |L(s)| < n$ otherwise,
when the start state is excluded.

By iterating
through all supporters of all states we effectively iterate through all symbols
in the input. This is because, for each state $s$, each supporter $i$ in $L(s)$ corresponds to a
...
\begin{lemma}[]~\label{lem-supporters}
  For a reduced set of keywords, the sum $\sum_s |L(s)|$ over each state $s$, except the start state,
  is $n$, where $n$ is the
...
\begin{proof}[Proof (by induction)]
  ...
  there are $|x|+1$ states, where $x$ is the keyword, and $|L(s)| = 1$ for each $|x|$ states,
  producing $\sum_s |L(s)| = |x| = n$.

  Let there be an AC-machine $AC$ constructed from a reduced set of keywords of size $M$ and a
  corresponding supporters set function $L()$. Let $y$ be a keyword not presented in $AC$.
  When $y$ is added to $AC$ a path from start state to a leaf is branched at depth $d$ such that
  $y[1...d]$ is the longest prefix of $y$ that has a corresponding state in $AC$. The $L(q)$ for
  each new state $q$ consists of only the index of the keyword $y$. The sets of supporters $L(r)$,
  for each existing state $r$ are appended with the index of the keyword $y$.

  In other words, the sets of supporters are increased by one for each $|y|$ states between
...
of the for loop is a constant time operation and by Lemma \ref{lem-supporters} $\sum_s |L(s)| = O(n)$,
...
show that Algorithm \ref{ac-goto}, \ref{ac-fail} and \ref{ukk-pre} has the same time complexity depending
...
a compile-time macro OPTIMIZE\_LINKS that changes the data structure that is used to the children of each
...
phases of the execution and their time usages. Without the INFO macro the program outputs only
...
execute the binary with a single input file. The file should contain a single byte encoded text with
line feed (ASCII 10) delimited keywords. The file should end with a line feed. Note that the input
...
The 'N' characters were relatively rare in this file (3506 out of 3849544 lines contain at leas one 'N').
...
\footnotetext{Gentoo Linux is a so called rolling release distribution. That means it does not have any version numbers.}
...
with keyword lengths $\leq 256$ there were $31/700$ discarded values when time usages were
measured in Haapa.
...
and RBD versions.
%In practice, we can limit the number of our implementations to two.
...
% TODO units? caption ? table should be figure. 
%\afterpage{%
...
\vspace{-1.5cm}
To further illustrate runtimes, we also sampled a set of
instances with linearly increasing size from 100,000 lines to 2,000,000 lines in 100,000-line
steps. This set was then run in Env2. Every instance was run only once and no precise values
...
\caption{Comparison of the runtimes between our and reference implementation with DNA data consisting
  ...
  implementation. Black line market with circles shows the runtimes of our implementation. Runtimes are
...
\begin{center}
  ...
  \caption{RLZ Compression sizes in mebibytes of the original PizzaChili files with different dictionaries.
    The best compressions are bolded. The upper numbers result when
    the RLZ dictionary is a common superstring of the corresponding file. The common superstring
    is generated from the files randomly sampled with parameters specified in the column title.
    The lower numbers result
    when the dictionary is a string obtained by concatenating random samples together such that both
...
number of RLZ factors becomes. We also see that the number of factors tends to decrease faster
...
better with lesser number of factors. This is because the size of the dictionary tends to grow
more rapidly than the number of factors decreases. This is especially true with the SCS dictionary
...
there is (a local) minimum for the compression with different sampling parameters.
...
%-brief introduction. what is in this chapter and in what order?
%In this chapter we recap the contents of this thesis.
...
% explicitely answer the research questions
