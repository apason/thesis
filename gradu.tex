%% TODO
% Jos ehtii. muuttujanimet ja todistusten joissa viitataan koodiin yhtenäistäminen. (algo 2)
...
\keywords{shortest common superstring, relative lempel-ziv, implementation}
...
\begin{algorithm}[t]
  ...
  \hspace*{\algorithmicindent} \textbf{Preconditions:} Initially $g(q,\sigma)$ is undefined for each state and symbol
      ...
      \begin{algorithmic}[1]
        ...
        \Function{calculateGotoFunction}{$R$}
          \State $q\gets 1$
          \For{$i \gets 1$ until $m$}
            \State $insert(s_i)$
          \EndFor
          \For{$\sigma$ s.t. $g(1,\sigma)$ is not defined }
      ...
      %\item[]
        ...
        \Function{insert}{$s = \sigma_0\sigma_1\cdots\sigma_{k-1}$}
          \State $p\gets 1$
          \State $j\gets 0$
          \While{$g(p, \sigma_j)$ is defined}
            \State $p\gets g(p,\sigma_j)$
            \State $j\gets j+1$
          \EndWhile
          \For{$l\gets j$ until $k-1$}
            \State $q\gets q+1$
...
\end{algorithm}
  ...
  The procedure \textit{insert} inserts a single keyword to the goto function. The idea in the lines
...
%   [scale=.8]
  ...
  \end{tikzpicture}
  \caption{Illustration of the AC-machine after the first key is added in Example 3.5.} \label{fig-ac-step2}
  \end{figure}
...
%   [scale=.8]
  ...
  \end{tikzpicture}
  \caption{Illustration of the AC-machine after the second key is added in Example 3.5.} \label{fig-ac-step3}
\end{figure}
...
%   [scale=.8]
  ...
  \end{tikzpicture}
  \caption{The complete goto graph for the set of keywords $R=\{$``baa'', ``baba'', ``abab'', ``aab''$\}$.} \label{fig-ac-goto}
\end{figure}
  ...
  In an AC-machine the depth of a state $q$, denoted by $d(q)$, is the depth of the node (number of
  edges from root to that node) that corresponds to
  the state in the goto graph. This is equivalent to the length of the string that represents the
  state, i.e., if a node $q$ in the goto graph is represented by the string $s$, then $d(q) = |s|$.
...
%   [scale=.8]
  ...
  \end{tikzpicture}
  \caption{The point of Example 3.6 when $f(q)$ is defined for all states $q$ with depth $\leq$ 1.} \label{fig-ac-step5}
\end{figure}
  

  %% NOTE. The number of example is hardcoded here! Cannot ref to testexample envs for some reason?
  In Example 3.6 the value for the failure function was always found in a single
  ...
  \begin{algorithm}[]
    ...
    \begin{algorithmic}[1]
      \Function{calculateFailureFunction}{$g$}
        ...
        \For {each $\sigma$ s.t. $g(1,\sigma) = q \neq 1$}
        ...
        \While{$queue \neq empty$}\\
          ...
          \For{each $\sigma$ s.t. $g(r,\sigma) = q$ is defined}
            \State $queue\gets queue \cup \{q\}$
            \State $p\gets f(r)$
            \While{$g(p,\sigma)$ is undefined}
              \State $p\gets f(p)$
            \EndWhile
    ...
    \begin{algorithmic}[1]
      \Function{calculateAuxiliaryFunctions}{$g, f, R$}
        \For {$i = 1,...,m$} \\
          ...
          \For {$j = 0,...,k-1$}
            ...
            \If {$j = k-1$}
              \State $F(i)\gets q$
              \State $E(q)\gets i$
              \If {$q$ is not a leaf of the AC machine}
                \State $F(i)\gets 1$
              \EndIf
            \EndIf
          \EndFor
        ...
        \While{$queue\neq empty$}\\
          ...
          \State $queue\gets queue \backslash \{r\}$
          %% ERRATA: Cyclic graph!          
          \For {each $q$ that is a child of $r$ and $q\neq r$} \footnotemark[2]
...
the state represented by $s_i$ has an incoming failure transition from any other state.\vspace{0.5cm}
  ...
  \footnotetext[1]{In the original algorithm this line was $L(q)\gets L(q) \cdot \{j\}$.}
  \footnotetext[2]{To prevent infinite recursion the inequality $q\neq r$ was added.}
  ...
  \begin{algorithm}[!ht]
    ...
    \begin{algorithmic}[1]
      \Function{createPath}{$f,F,L,b,B,R$}
        ...
        \While {$q\neq 1$}
          \If {$P(q)$ is not empty}
            \For {each $j$ in $L(q)$ s.t. $forbidden(j) = false$}
              %% ERRATA: Check emptiness
              \If {$P(q)$ is empty} \footnotemark[1]
                \State \textbf{break}
              \EndIf
              \State $i\gets$ the first element of $P(q)$
              \If {$FIRST(i) = j)$}
                \If {$P(q)$ has only one element}
...
remains valid and lines 24 and 25 update the $FIRST$ and $LAST$ functions accordingly.\footnotetext[1]{This line is not present in the original algorithm.}
