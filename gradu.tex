...
% TODO: still not happy with the title. Should have "implementation" and "approximation" in it. 
\title{Efficient Computation of Shortest Superstrings with Application to Relative Lempel-Ziv Dictionary Construction}
...
\begin{definition}[\textsc{Overlap and Maximal Overlap}]~\label{def-overlap}\\
  ...
  if and only if $U$ is a prefix of $S$ and a suffix of $R$.
...
\begin{definition}[\textsc{Common superstring}]~\label{def-cs}\\
  Let $R = \{S_1,...,S_n\}$ be a set of strings and $T$ be a string.
  $T$ is a common superstring of $R$ if and only if $T$ is a superstring
...
\begin{definition}[\textsc{Shortest Common Superstring}]~\label{def-scs}\\
  Let string $T$ be a common superstring of a set of strings $R$.
  The string $T$ is a shortest common superstring of $R$ if and only if there does not exist
  a common superstring of $R$ which is shorter than $T$.
\end{definition}
...
\end{figure}
  ...
  The algorithm starts from line 2 where the root node is created. Figure \ref{fig-ac-step1}
