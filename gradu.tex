...
is proven using the \textsc{Longest Hamiltonian Path} problem. Also, Ukkonen's algorithm conceptually
resolves the SCS problem by constructing the Hamiltonian path.
...
This chapter contains four pseudo code blocks which essentially forms the algorithm as whole.
For each of those algorithms, we first discuss their principles in a higher level. Then the pseudocode
...
were originally presented. Each such modification is noted to avoid confusion of how the original
authors defined the algorithms.
...
is $s_1s_2\cdots s_m$.
The following greedy heuristic can be used to construct an approximation of the shortest
common superstring.

\begin{enumerate}
\item Examine the set and remove the two strings $s_i$ and $s_j$ that have the longest overlap among all pairs of strings $(s_i, s_j)$ with $i\neq j$. Note that the longest overlap may be an empty string. If there are multiple pairs of strings with the longest overlap, the decision can be made arbitrarily.
...
complete digraph from the set of strings as follows. Let $w(e)$ denote the weight of the edge $e$.
...
More formally $G_R=(V_R=\{v_i=s_i\in R\}\cup\{v_{start},v_{end}\}, E_R=\{(v_i,v_j) : i\neq j\}\cup\{(v_{start},v_{i})\}\cup\{(v_i,v_{end})\},w)$
...
The algorithm \ref{ukk-h} starts by setting the initial values for the dynamic functions
$P, FIRST, LAST$ and $forbidden$ in the for loop starting at lines 2. In lines 4 to 6 the algorithm
...
to $i$, indicating there is no edges yet selected. The else clause in lines 7 to 8 is
...
first state to process is set in line 11. Note that setting $s=B$ would cause one more unnecessary
...
In line 13 we ensure that $P(s)$ is not empty since otherwise there are no allowed overlaps.
...
size of $P(s)$ and $\{i\in L(s) : forbidden(i) = false\}$. In lines 15 to 17 we check that
...
(n line 19). If the edge $(s_i,s_j)$ would create a cycle we check in line 20 if there exists
...
In line 26 the edge is selected. The lines 27 ensures the definition for $forbidden$ function
remains valid and lines 29 and 30 updates the $FIRST$ and $LAST$ functions accordinly. In line
...
Before that in line 32 the $P(s)$ is catenated to $P(f(s))$ so the possibly remaining unused
...
How is that condition in line 14 resolved in linear time ????




\begin{theorem}[]~\label{thm-h-time}
  The time complexity of algorithm \ref{ukk-h} equals the time complexity of
  algorithm \ref{ac-goto}
\end{theorem}

The theorems \ref{thm-time-goto}, \ref{thm-time-failure}, \ref{thm-pre-time} and \ref{thm-h-time}
conclude that every algorithm presented in this thesis have the same time complexity that depends
on the implementation of the goto function for the AC-machine. As discussed earlier the implementation
of the goto function can be done in $O(n)$ using direct indexing over two dimensional array. Other
feasible possibility, especially for the larger alphabets, is to use balanced binary trees with
$O(log(n))$ time complexity. This conclusion leads us to the following theorem.
